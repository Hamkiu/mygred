var CORKUI = function() {
    let layoutName = 'Horizontal Dark Menu';
    // let _ThemeColors;
    ...
    var toggleFunction = {
        ...
        topbar: function(_toggleMode) {
            const toggleMode = _toggleMode; // 'hover' or 'click'
            ...
            function setupDropdown(root) {
                ...
                // Function to show dropdown using Floating UI
                ...
                const submenus = root.querySelectorAll('.admin-submenu');
                submenus.forEach(sub => {
                    const submenuBtn = sub.querySelector('.admin-dropdown-item');
                    const submenu = sub.querySelector('.admin-dropdown-menu');
            
                    sub.addEventListener('mouseenter', () => {
                        submenu.classList.add('show');
                        submenuBtn.classList.add('open');
                
                        window.FloatingUIDOM.computePosition(submenuBtn, submenu, {
                            placement: 'right-start',
                            middleware: [window.FloatingUIDOM.offset(0), window.FloatingUIDOM.flip(), window.FloatingUIDOM.shift()],
                        }).then(({ x, y }) => {
            ...
            function closeAllDropdowns() {
                ...
                document.querySelectorAll('.admin-menu-toggle.open, .admin-dropdown-item.open').forEach(btn => {
    ...
    var inBuiltfunctionality = {
        ...
        mainCatActivateScroll: function() {
            ...
            if (document.querySelector('.sidebar-wrapper .menu-categories')) {
            
                const ps = new PerfectScrollbar('.sidebar-wrapper .menu-categories', {
                    wheelSpeed:.5,
                    swipeEasing:!0,
